...
export const useStreamWebRTC = (
   ...
   *
   * This is the REAL hardware stream.
   *
   ...
   * - AI video effects
   *
   * so cleanup can safely stop the actual camera/microphone.
   */
  ...
  /*
   * Stream containing the audio that WebRTC uses.
   *
   * The video track sent by WebRTC may come from AIEffects
   * instead of this stream.
   */
  ...
  /*
   * ============================================================
   * AI VIDEO EFFECT STATE
   * ============================================================
   */

  /*
   * Original camera video track.
   *
   * This is kept so we can restore the camera when AI Effects
   * are disabled.
   */
  const rawVideoTrackRef = useRef(null);

  /*
   * Current processed video track generated by AIEffects.
   */
  ...
  /*
   * ============================================================
   * AI VIDEO TRACK REPLACEMENT
   * ============================================================
   *
   * This is the important connection between AIEffects and
   * WebRTC.
   *
   * Camera:
   *
  ...
  const replaceLocalVideoTrack =
    useCallback(
      async nextProcessedTrack = null => {
        ...
        /*
         * If AIEffects supplies a valid video track,
         * use it.
         *
         * Otherwise restore the original camera track.
         */
        const targetTrack =
          nextProcessedTrack &&
          nextProcessedTrack.kind === 'video' &&
          nextProcessedTrack.readyState !== 'ended'
            ? nextProcessedTrack
            : rawTrack;

        if (!targetTrack) {
          console.warn(
            '⚠️ [WebRTC] No local video track available for replacement.'
          );

          return false;
        }

        processedVideoTrackRef.current =
          nextProcessedTrack &&
          nextProcessedTrack.kind === 'video' &&
          nextProcessedTrack.readyState !== 'ended'
            ? nextProcessedTrack
        ...
        /*
         * Replace the video sender on EVERY active peer.
         */
        for (
          const [
            peerId,
          ...
          try {
            ...
            if (!videoSender) {
              console.warn(
                '⚠️ [WebRTC] No video sender found for peer:',
                peerId
              );
            ...
            console.log(
              '🎨 [WebRTC] Video track replaced for peer:',
              peerId,
              {
                processed:
                  targetTrack !==
            ...
            console.warn(
              '⚠️ [WebRTC] Failed replacing video track for peer ' +
                peerId +
                ':',
              error?.message ||
                error
        ...
        console.log(
          '🎨 [WebRTC] Local video track updated:',
          {
            processed:
              targetTrack !== rawTrack,
            trackId:
              targetTrack.id,
            peersUpdated:
              replacementCount
  ...
  /*
   * Keep the latest replacement function in a ref.
   *
   * This allows peer creation to use the newest AI video
   * track without creating dependency problems.
   */
  const replaceLocalVideoTrackRef =
    useRef(replaceLocalVideoTrack);

  useEffect(() => {
    replaceLocalVideoTrackRef.current =
      replaceLocalVideoTrack;
  }, [replaceLocalVideoTrack]);

  /*
   * ============================================================
   * SYNCHRONIZE AIEFFECTS TRACK
   * ============================================================
   *
   * StreamDashboard will pass:
   *
   *   processedVideoTrack
   *
   * from AIEffects.
   *
   * Every time the effect changes or the engine produces a
   * replacement track, this updates all active WebRTC peers.
   */
  ...
  useEffect(() => {
     ...
     * - processed microphone audio
     *
     * AIEffects is handled separately through
     * processedVideoTrack.
     */
    const attachLocalStream =
      stream => {
        ...
        /*
         * Capture the ORIGINAL camera track before AIEffects
         * can replace anything in WebRTC.
         */
    ...
    const requestMedia =
      async () => {
        ...
        try {
           ...
           *     Processed microphone audio
           *
           * AI video remains separate and is connected later
           * through processedVideoTrack.
           */
          ...
          /*
           * The raw camera track must also receive the
           * camera state.
           */
          rawVideoTrackRef.current &&
            (rawVideoTrackRef.current.enabled =
              !isCameraOff);

          /*
           * If AIEffects has already produced a track,
           * synchronize it now.
           */
          if (
            processedVideoTrackRef.current
          ) {
            processedVideoTrackRef.current.enabled =
              !isCameraOff;
          }
  ...
  useEffect(() => {
    ...
    /*
     * Keep voice engine synchronized.
     */
    try {
      if (
        liveVoiceEngine &&
     ...
     * Camera:
     *
     * Always update BOTH tracks:
     *
     * - original hardware track
     * - processed AIEffects track
     */
    if (
      rawVideoTrackRef.current
    ) {
  ...
  const createPeerConnection =
    useCallback(
      targetSocketId => {
         ...
         *
         * Video:
         *   processedVideoTrack if available
         *   otherwise raw camera track
         */
        ...
        if (stream) {
          ...
          stream
            ...
            .forEach(track => {
              try {
                ...
                console.log(
                  '📡 [WebRTC] Added local audio track to peer ' +
                    targetSocketId
                );
              } catch (error) {
                console.warn(
                  '⚠️ [WebRTC] Failed adding audio track:',
                  error?.message ||
                    error
                );
              }
           ...
           *
           * IMPORTANT:
           * Do NOT automatically add the original video
           * and the processed video.
           *
           * Only ONE video track should be sent.
           */
          const rawTrack =
            rawVideoTrackRef.current ||
          ...
          const videoTrack =
            effectTrack &&
            effectTrack.kind === 'video' &&
            effectTrack.readyState !== 'ended'
              ? effectTrack
              : rawTrack;
          ...
          if (videoTrack) {
            try {
              ...
              console.log(
                '📡 [WebRTC] Added local video track to peer ' +
                  targetSocketId,
                {
                  processed:
                    videoTrack ===
              ...
              console.warn(
                '⚠️ [WebRTC] Failed adding video track:',
                error?.message ||
                  error
              );
            }
  ...
  return {
    ...
    /*
     * Allows StreamDashboard/AIEffects to explicitly replace
     * or restore the outgoing video track.
     */
    replaceLocalVideoTrack
  };
};
