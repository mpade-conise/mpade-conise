...
const StreamDashboard = () => {
  ...
  const [battleScores, setBattleScores] = useState({ host: 1420, challenger: 980 });
  ...
  // Handle tiered gifts (< 50 coins = emoji particle in video, >= 50 coins = full overlay)
  useEffect(() => {
    if (activeGift) {
      if (activeGift.price < 50) {
        setActiveSmallGift(activeGift);
        // Clear activeGift from taking over the screen
        setActiveGift(null);
      }
    }
  ...
  return (
    <div className="h-[100dvh] w-full bg-zinc-950 text-white overflow-hidden relative font-sans flex flex-row">
      ...
      <div className="relative flex-1 h-full overflow-hidden">
        ...
        <div className="absolute top-0 left-0 right-0 z-[60] p-4 pt-8 bg-gradient-to-b from-black/90 via-black/40 to-transparent flex flex-col gap-2.5">
          ...
          <div className="flex justify-start pl-1 pointer-events-auto">
            <LiveStreamGoalBar streamId={streamId} isHost={true} />
          </div>
            ...
            <div className="w-full max-w-lg mx-auto pt-1 pointer-events-auto">
              <MultiHostPKBattleBar 
                hosts={[
                  ...
                  ...(activeCoHosts.length > 0 ? activeCoHosts.map((c, i) => ({
                    id: c.id || `cohost-${i}`,
                    username: c.username || `Host ${i+2}`,
                    avatar: c.avatar_url,
                    score: Math.max(200, (battleScores.challenger || 980) - (i * 120)),
                    topGifters: [
                  ...
                  })) : [
                    {
                      ...
                      score: battleScores.challenger || 980,
                      topGifters: [
        ...
        <AnimatePresence>
          {pendingRequests.length > 0 && activePanel !== 'guests' && (
            <div className="absolute top-28 left-1/2 -translate-x-1/2 z-[65] w-full max-w-sm px-4">
        ...
        {/* 2. DYNAMIC STAGE VIEWPORT WITH 50/50 SPLITTING LINE & BATTLE OVERLAYS */}
        ...
        {/* Control Console Dock Bar & Borderless Chat at bottom */}
        <div className="absolute bottom-0 left-0 right-0 z-50 p-4 space-y-3 pointer-events-none">
          <div className="h-52 w-full max-w-[340px] pointer-events-auto overflow-y-auto floating-chat-container">
