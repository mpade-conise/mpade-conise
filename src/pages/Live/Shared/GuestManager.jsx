...
import { ArrowLeft, Radio, Video, Mic, X, Users, UserCheck, UserX } from 'lucide-react';
...
const GuestManager = ({ 
  ...
  // Handle Accept Request (Updates Supabase, Socket, and local grid state)
  const handleAcceptRequest = async (request, assignedMode) => {
    ...
    // 1. Update Supabase record status to 'approved'
    ...
    if (error) {
      console.error("Failed to approve guest request in DB:", error);
      return;
    }
