...
const peerReadyHandledRef = useRef(false);
...
if (
  ...
  try {
    ...
    console.warn("Failed sending hangup signal:", error);
  }
}
...
peerReadyHandledRef.current = false;
...
const initProfiles = async () => {
  ...
  try {
    ...
    if (!peerUserId || peerUserId === 'undefined') {
      console.error("Invalid peerUserId.");
    ...
    if (!cancelled && !error && data && isMountedRef.current) {
      setPeerProfile(data);
    }
  } catch (error) {
...
console.log(
  `Setting up call signaling as [${callRole}] for room ${roomId}`
);
...
const createAndSendOffer = async () => {
  ...
  try {
    ...
    console.log("WebRTC offer sent once:", roomId);
  } catch (error) {
    offerSentRef.current = false;

    console.error(
      "Failed creating WebRTC offer:",
      error
    );
  }
};
...
const initializeMediaAndSignaling = async () => {
  try {
    ...
    socket.on('connect', async () => {
      ...
      console.log(
        `Signaling connected: ${socket.id} | room: ${roomId}`
      );
      ...
      if (callRole === 'caller') {
        ...
        if (!callInviteSentRef.current) {
          ...
          console.log(
            "Incoming call signal sent once."
          );
        }
      } else {
        setCallStatus("Awaiting Connection...");

        if (!peerReadyHandledRef.current) {
          peerReadyHandledRef.current = true;
    ...
    socket.on(
      ...
      async ({ answer }) => {
        ...
        if (
          ...
          console.warn(
            "Ignoring answer because peer connection is not waiting for an answer."
          );
          return;
        }
    ...
    console.error(
      "Media/signaling initialization error:",
      error
    );
...
return ( <div className="fixed inset-0 bg-zinc-950 text-white flex flex-col items-center justify-between p-4 sm:p-6 font-sans select-none overflow-hidden"> <div className="w-full max-w-lg flex justify-between items-center bg-white/5 px-4 py-3 rounded-2xl border border-white/10 backdrop-blur-md z-30 shadow-xl"> <div className="flex items-center gap-2"> <Shield
         size={16}
