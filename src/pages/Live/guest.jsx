...
import { 
  ...
  X, UserPlus, Shield, Mic, MicOff, VideoOff, 
  ...
  const handleStartGuestStream = async () => {
    ...
    try {
      ...
      if (error) {
        ...
        // Emit WebRTC Room Creation event to Socket server
  ...
  return (
    <div className="h-[100dvh] bg-[#030308] text-white flex flex-col justify-between overflow-hidden font-sans relative">
      ...
      <div className="w-full z-50 p-4 sm:p-6 flex justify-between items-start pointer-events-none">
        <button onClick={() => navigate(-1)} className="p-2 sm:p-2.5 bg-black/40 backdrop-blur-xl rounded-full border border-pink-500/30 text-pink-300 hover:border-pink-500/80 pointer-events-auto transition-all">
        ...
        <div className="flex flex-col gap-2 sm:gap-3 pointer-events-auto items-end">
           <ControlIconButton icon={<Settings size={18}/>} label="Settings" />
           <ControlIconButton icon={<Sparkles size={18}/>} label="Effects" />
           <ControlIconButton icon={<Wand2 size={18}/>} label="Beautify" />
           <ControlIconButton 
      ...
      <div className="flex-1 relative p-3 sm:p-4 overflow-y-auto no-scrollbar z-20">
        <div className="grid grid-cols-2 md:grid-cols-4 gap-2.5 sm:gap-3 max-w-4xl mx-auto h-full auto-rows-fr">
          ...
          {/* HOST PANEL */}
          <div className="relative rounded-2xl overflow-hidden bg-zinc-950 border-2 border-pink-500/80 col-span-2 row-span-2 min-h-[180px] sm:min-h-[220px]">
            {isCamOn ? (
            ...
            <div className="absolute top-3 left-3 bg-pink-600/80 backdrop-blur-md px-2.5 py-1 rounded-full text-[10px] font-black uppercase flex items-center gap-1 border border-pink-400/50">
            ...
            <div 
              ...
              className={`relative rounded-2xl border flex flex-col items-center justify-center p-2 min-h-[100px] sm:min-h-[120px] transition-all overflow-hidden ${
                slot.isLocked 
                  ...
                  : 'bg-black/40 backdrop-blur-xl border-cyan-500/30 text-cyan-300 hover:border-cyan-400'
              }`}
            >
              ...
              {slot.occupant ? (
                ...
                <div className="flex flex-col items-center gap-1 opacity-70">
                  ...
                  <span className="text-[9px] font-black uppercase text-cyan-200/60">{slot.label}</span>
                </div>
              )}
            </div>
          ))}
...
const ControlIconButton = ({ icon, label, onClick }) => (
  <button onClick={onClick} className="flex flex-col items-center gap-1 group">
    <div className="p-2 sm:p-3 bg-black/40 backdrop-blur-xl rounded-xl border border-cyan-500/30 text-cyan-300 group-hover:bg-pink-600 transition-all">
      {icon}
    </div>
