...
const VideoCall = () => {
  ...
  const processIceQueue = async () => {
    ...
    if (!pc || !pc.remoteDescription || iceQueueRef.current.length === 0) {
      return;
    }
  ...
  const removeRealtimeChannels = async () => {
    ...
    for (const channel of channels) {
      try {
        ...
        console.warn("Failed removing realtime call channel:", error);
      }
    }
  };

  const performCleanup = async (shouldNotifyPeer = true, shouldNavigate = true) => {
    ...
    if (shouldNotifyPeer && socket && socket.connected && hasValidPeer) {
      const cancelPayload = {
        roomId,
        to: peerUserId,
      ...
      try {
        ...
        console.warn("Failed sending call cleanup signals:", error);
      }

      try {
        const realtimeChan = supabase.channel(`user-call-signals-${peerUserId}`);

        realtimeChannelsRef.current.push(realtimeChan);
        ...
        realtimeChan.subscribe((status) => {
          if (status === 'SUBSCRIBED') {
            realtimeChan.send({
              ...
              console.warn("Failed sending call cancellation broadcast:", error);
            });
          }
        });
      } catch (error) {
        console.warn("Failed creating call cancellation channel:", error);
      }
    }
    ...
    if (screenTrack) {
      try {
        ...
        console.warn("Failed stopping screen track:", error);
      }
    ...
    if (localStream) {
      try {
        ...
        console.warn("Failed stopping local media tracks:", error);
      }
    ...
    if (pc) {
      try {
        ...
        console.warn("Failed closing peer connection:", error);
      }
    ...
    if (socket) {
      try {
        ...
        console.warn("Failed closing signaling socket:", error);
      }
    ...
    if (shouldNavigate && !hasNavigatedRef.current && isMountedRef.current) {
  ...
  useEffect(() => {
    const initProfiles = async () => {
      console.log(
        "Incoming Call Routing State Check -> peerUserId:",
        peerUserId,
        "| Role Parameter:",
        URLRole
      );
      ...
      try {
        ...
        if (!peerUserId || peerUserId === 'undefined') {
          console.error(
            "Aborting profile fetching: peerUserId is undefined."
          );
        ...
        console.error("Failed initializing call profile:", error);
  ...
  useEffect(() => {
    if (!currentUserId || !peerUserId || peerUserId === 'undefined') {
      return;
    }
    ...
    const createAndSendOffer = async () => {
      ...
      try {
        ...
        if (!isComponentMounted || pc.signalingState === "closed") {
          return;
        }

        await pc.setLocalDescription(offer);

        if (!isComponentMounted || !socket.connected) {
          return;
        }
        ...
        console.error("Failed creating signaling offer:", error);
      }
    };
      ...
      try {
        ...
        socket.on('connect', async () => {
          ...
          if (callRole === 'caller') {
            ...
            try {
              const realtimeChan = supabase.channel(
              ...
              realtimeChan.subscribe((status) => {
                if (
                  ...
                  realtimeChan.send({
                    ...
                    console.warn(
                      "Incoming call broadcast failed:",
                      error
                    );
                  });
                }
              ...
              console.warn(
                "Failed creating incoming call fallback channel:",
                error
              );
            }
        ...
        socket.on('peer_ready', async () => {
          ...
          if (callRole === 'caller' && pcRef.current) {
            console.log(
              "Peer is ready. Dispatching WebRTC offer."
            );
        ...
        socket.on(
          ...
          async ({ candidate }) => {
            if (!isComponentMounted || !candidate) {
              return;
            }
        ...
        socket.on('peer_hung_up', async () => {
          ...
          await performCleanup(false, true);
        });
        ...
        socket.on('in_call_reaction_burst', (data) => {
          if (!isComponentMounted || !data?.emoji) {
            return;
          }
    ...
    return () => {
      ...
      if (cleanupRef.current) {
        cleanupRef.current(false, false);
      }
    };
        ...
        screenTrack.onended = () => {
          if (
            ...
            if (
              ...
              const cameraSender = pcRef.current
                .getSenders()
                .find((s) => s.track?.kind === 'video');

              if (cameraSender) {
                cameraSender.replaceTrack(cameraTrack).catch(
                  (error) => {
                    console.warn(
  ...
  const cleanUpCall = async () => {
    await performCleanup(true, true);
  };
