  ...
  useEffect(() => {
    const initProfiles = async () => {
      ...
      if (!peerUserId || peerUserId === 'undefined') {
        console.error(
          "❌ Aborting profile fetching loop: peerUserId url parameter resolved to an undefined state."
        );
      ...
      try {
        ...
        socket.on('peer_ready', async () => {
          ...
          if (callRole === 'caller' && pcRef.current) {
            console.log(
              "⚡ Peer is ready in room. Dispatching WebRTC offer."
            );
        ...
        socket.on('webrtc_offer_received', async ({ offer }) => {
          if (!isComponentMounted || !pcRef.current) {
            return;
          }
          ...
          try {
            ...
            console.error(
              "Failed executing structural handshake offer loop:",
              err
            );
          }
        });
        ...
        socket.on('webrtc_answer_received', async ({ answer }) => {
          ...
          try {
            ...
            console.error(
              "Failed setting up active remote answer specification:",
              err
            );
          }
        });
        ...
        socket.on('incoming_ice_candidate', async ({ candidate }) => {
          ...
          if (
            ...
            try {
              ...
              console.warn(
                "Skipped structural candidate node:",
                e
              );
            }
          } else {
