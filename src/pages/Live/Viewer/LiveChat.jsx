    ...
    <div className="flex flex-col h-full w-full pointer-events-none relative z-50">
      ...
      {!hideMessages && (
        <div className="flex-1 overflow-y-auto px-4 space-y-3 pointer-events-auto hide-scrollbar">
          {messages.length === 0 ? (
            <div className="text-cyan-400/50 text-[10px] p-4 italic drop-shadow-[0_0_6px_rgba(6,182,212,0.4)]">
            ...
            messages.map((m) => (
              <div key={m.id} className="flex items-start gap-2 max-w-full">
                {/* Avatar */}
                <div className="w-7 h-7 rounded-full bg-transparent flex-shrink-0 overflow-hidden mt-0.5">
                  {m.profiles?.avatar_url && (
                    <img src={m.profiles.avatar_url} className="w-full h-full object-cover rounded-full" alt="" />
                  )}
                </div>
                {/* Transparent Container without Box, Border, or Box Shadow */}
                <div className="flex flex-col bg-transparent p-0 border-none shadow-none">
      ...
      <div className="p-4 pointer-events-auto">
        <form onSubmit={sendMessage} className="relative flex items-center">
          {/* Input Box - Borderless with Transparent Background */}
          <input 
            type="text"
            value={input}
            onChange={(e) => setInput(e.target.value)}
            placeholder="Add comment..."
            className="w-full bg-black/20 border-none rounded-full py-2.5 pl-5 pr-12 text-sm text-cyan-100 placeholder:text-cyan-200/40 focus:outline-none transition-all shadow-none"
          />
          {/* Send Button */}
          <button 
            type="submit" 
            disabled={!input.trim()}
            className="absolute right-1.5 w-8 h-8 bg-[#fe2c55] rounded-full flex items-center justify-center text-white shadow-[0_0_12px_rgba(254,44,85,0.7)] hover:shadow-[0_0_18px_rgba(254,44,85,0.9)] active:scale-90 transition-all disabled:opacity-50 disabled:shadow-none"
          >
