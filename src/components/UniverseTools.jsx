...
const [isGuestSlotOpen, setIsGuestSlotOpen] = useState(false);
const [activeCohost, setActiveCohost] = useState(null);
              ...
              <div className="space-y-4">
                {myVideos.length > 0 ? myVideos.map(video => (
                  <motion.div 
                    ...
                    className="bg-zinc-900/50 p-3 rounded-[24px] border border-white/5 flex gap-4 items-center cursor-pointer group hover:border-cyan-500/30 transition-all"
                  >
                    <div className="w-16 h-20 bg-zinc-800 rounded-xl overflow-hidden relative shadow-lg">
                      {video.thumbnail_url ? (
                        <img 
                          src={video.thumbnail_url} 
                          className="w-full h-full object-cover" 
                          alt="thumb" 
                          onError={(e) => { e.target.src = 'https://via.placeholder.com/150/111/fff?text=Video'; }}
                        />
                      ) : (
                    ...
                    <div className="flex-1">
                       <h4 className="text-xs font-bold truncate w-40 italic">{video.caption || "Untitled Video"}</h4>
                       <div className="flex gap-3 mt-2">
                          <span className="flex items-center gap-1 text-[10px] text-zinc-500 font-bold"><Eye size={10} className="text-cyan-400"/> {video.views_count || 0}</span>
                          <span className="flex items-center gap-1 text-[10px] text-zinc-500 font-bold"><Heart size={10} className="text-rose-500"/> {video.likes_count || 0}</span>
                       </div>
                    </div>
                    <ChevronRight size={18} className="text-zinc-700 group-hover:text-cyan-400 transition-colors" />
                  </motion.div>
                )) : (
        ...
        {selectedVideo && (
          <>
            ...
            <motion.div 
              ...
              className="fixed bottom-0 left-0 right-0 bg-zinc-900 border-t border-white/10 rounded-t-[40px] z-[201] p-8 pb-12 max-h-[85vh] overflow-y-auto"
            >
              <div className="w-12 h-1.5 bg-zinc-800 rounded-full mx-auto mb-8" />
              <div className="flex items-start gap-6 mb-10">
                <div className="w-24 h-32 bg-zinc-800 rounded-2xl overflow-hidden shrink-0 border border-white/10 shadow-2xl">
                  <img src={selectedVideo.thumbnail_url} className="w-full h-full object-cover" alt="prev" />
                </div>
                <div>
                  <h2 className="text-xl font-black italic tracking-tight leading-tight">{selectedVideo.caption || "Untitled Content"}</h2>
                  <p className="text-[10px] font-bold text-zinc-500 uppercase tracking-[2px] mt-2">Deep Insights Breakdown</p>
                  <div className="flex gap-4 mt-4">
                    <div className="text-center">
                      <p className="text-lg font-black italic">{selectedVideo.views_count || 0}</p>
                      <p className="text-[8px] text-zinc-500 uppercase font-bold">Views</p>
                    </div>
                    <div className="text-center">
                      <p className="text-lg font-black italic text-rose-500">{selectedVideo.likes_count || 0}</p>
              ...
              <div className="grid grid-cols-2 gap-4 mb-8">
                 <div className="bg-white/5 p-6 rounded-[28px] border border-white/5">
                    <p className="text-[9px] font-black text-cyan-500 uppercase tracking-widest mb-1">Retention</p>
                    <p className="text-2xl font-black italic">64%</p>
                 </div>
                 <div className="bg-white/5 p-6 rounded-[28px] border border-white/5">
                    <p className="text-[9px] font-black text-yellow-500 uppercase tracking-widest mb-1">Avg Watch</p>
                    <p className="text-2xl font-black italic">0:42s</p>
                 </div>
              </div>
              <button 
                onClick={() => setSelectedVideo(null)}
                className="w-full py-5 bg-white text-black font-black uppercase tracking-[3px] text-xs rounded-[24px]"
              >
                Done Reading
              </button>
            </motion.div>
  ...
  {/* --- Cleaned NavButton for UniverseTools.jsx --- */}
<NavButton 
