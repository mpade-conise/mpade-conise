...
const UniverseTools = () => {
  ...
  const [drawerMode, setDrawerMode] = useState('metrics');
   ...
   * ---------------------------------------------------------
   *
   * This function intentionally fetches only the data required
   * by this page instead of using select('*').
   */
    ...
    try {
      ...
      if (!user) {
        if (mountedRef.current) {
          ...
          setFetchError('No authenticated creator session found.');
        }
      ...
      /*
       * Fetch profile and videos at the same time.
       * Only columns used by this page are requested.
       */
      ...
      const [profileResult, videosResult] = await Promise.all([
        ...
        supabase
          .from('videos')
          .select(
            'id, user_id, caption, video_url, views_count, likes_count, created_at'
          )
          .eq('user_id', user.id)
      ...
      if (profileResult.error) {
        console.error(
          '❌ Universe Studio profile fetch error:',
          profileResult.error
        );
      }
      ...
      if (videosResult.error) {
        console.error(
          '❌ Universe Studio videos fetch error:',
          videosResult.error
        );
      }
      ...
      const totalViews = videos.reduce(
        (total, video) =>
          total + toSafeNumber(video?.views_count),
        0
      );

      const totalLikes = videos.reduce(
        (total, video) =>
          total + toSafeNumber(video?.likes_count),
        0
      );
       ...
       * 1 coin = 0.10 MWK
       *
       * Keep this calculation because it is already part
       * of the current Universe Studio behavior.
       */

      const calculatedRevenue = rawCoins * 0.1;
      ...
      setStats({
        ...
        revenue: calculatedRevenue.toLocaleString(undefined, {
          minimumFractionDigits: 2,
          maximumFractionDigits: 2,
        }),
        coins: formatNumber(rawCoins),
      });

      /*
       * Pick a creator tip after successful synchronization.
       */
      ...
      /*
       * If one query failed, let the page remain usable,
       * but show a small non-blocking error.
       */

      if (profileResult.error || videosResult.error) {
        setFetchError(
          'Some creator data could not be synchronized. Try refreshing.'
        );
      }
    } catch (error) {
      console.error(
        '❌ Universe Studio Pipeline Sync Error:',
        error
      );
  ...
  const handleOpenDrawer = useCallback((video) => {
    if (!video) return;
    ...
    <div className="h-screen w-full bg-black text-white font-sans flex flex-col overflow-hidden relative selection:bg-cyan-500/20">
      ...
      <nav className="flex items-center justify-between px-6 py-5 border-b border-white/5 bg-black/60 backdrop-blur-2xl z-[100] shrink-0">
        ...
        <div className="flex items-center gap-4">
          ...
          <h2 className="text-xs font-black tracking-[4px] uppercase italic text-zinc-400">
            Universe Studio
          </h2>
        </div>
        ...
        <div className="flex items-center gap-4">
          ...
          <button
            ...
            disabled={syncing}
            className="p-1.5 rounded-lg hover:bg-white/5 active:scale-95 transition-all disabled:opacity-50"
            aria-label="Refresh creator statistics"
          >
            <RefreshCcw
              size={16}
      ...
      <AnimatePresence>
        {fetchError && (
          <motion.div
            ...
            exit={{ opacity: 0, y: -8 }}
            className="absolute top-[74px] left-1/2 -translate-x-1/2 z-[110] w-[90%] max-w-md"
          >
            <div className="px-4 py-2.5 rounded-xl bg-yellow-500/10 border border-yellow-500/20 backdrop-blur-xl">
              <p className="text-[8px] font-bold text-yellow-400 text-center uppercase tracking-wider">
                {fetchError}
              </p>
            </div>
          </motion.div>
              ...
              >
                ...
                <div className="p-5 bg-gradient-to-r from-cyan-600/10 to-purple-600/10 border border-cyan-500/20 rounded-[28px] flex gap-4 items-center">
                  ...
                  <div>
                    ...
                    <p className="text-xs font-medium text-zinc-300 mt-0.5 leading-relaxed">
                      {aiTip}
                    </p>
                  </div>
                  ...
                  <div className="bg-zinc-900/30 border border-white/5 rounded-[32px] p-6 backdrop-blur-md">
                    ...
                    <div className="flex justify-between items-center mb-6">
                      ...
                      <div>
                        ...
                        <p className="text-3xl font-black italic mt-1 font-mono">
                          {stats.views}
                        </p>
                      </div>

                      <div className="bg-emerald-500/10 text-emerald-400 px-2.5 py-0.5 rounded-full text-[9px] font-mono font-bold">
                        +12.4%
                      </div>
                    ...
                    <div className="flex gap-1.5 items-end h-16 pt-2">
                      ...
                      {[40, 70, 45, 90, 65, 80, 50, 60, 35, 75].map(
                        (height, index) => (
                          <div
                            key={`bar-${index}`}
                            className="flex-1 bg-cyan-500/10 rounded-t-md relative group h-full flex items-end"
                          >
                            <motion.div
                              initial={{ height: 0 }}
                    ...
                    myVideos.map((video) => (
                      ...
                      >
                        ...
                        <div className="flex-1 min-w-0">
                          ...
                          <h4 className="text-xs font-bold truncate text-zinc-200 italic">
                            {video.caption ||
                              'Untitled Stream Instance'}
                          </h4>
                        ...
                        <ChevronRight
                          size={16}
                          className="text-zinc-700 group-hover:text-cyan-400 transition-colors shrink-0 ml-2"
                        />
            ...
            {activeTab === 'monetization' && (
              ...
              >
                ...
                <div className="bg-gradient-to-br from-purple-900/40 via-blue-900/20 to-black p-8 rounded-[36px] text-center border border-purple-500/20 shadow-2xl relative overflow-hidden">
                  ...
                  <p className="text-[9px] font-black uppercase tracking-[3px] text-purple-400 opacity-80">
                    Withdrawable Balance
                  </p>

                  <h2 className="text-4xl font-black italic mt-2 tracking-tighter font-mono text-zinc-100">
                    K{stats.revenue}
                  </h2>
              ...
              >
                ...
                <div className="bg-zinc-900/30 border border-white/5 rounded-[32px] p-6 space-y-4">
                  ...
                  <GeoProgress
                    ...
                    percent={Math.min(
                      toSafeNumber(
                        String(stats.followers).replace(
                          /[^0-9.]/g,
                          ''
                        )
                      ),
                  ...
                  <GeoProgress
                    label="Reaction to Extraction Frequency"
                    percent={38}
            ...
            >
              ...
              <div className="flex items-start gap-4 mb-6 shrink-0 bg-white/[0.01] p-3 rounded-2xl border border-white/5">
                ...
                <div className="min-w-0 flex-1">
                  ...
                  <h2 className="text-base font-black italic tracking-tight truncate text-zinc-100">
                    {selectedVideo.caption ||
                      'Untitled Content Session'}
                  </h2>
                    ...
                    >
                      ...
                      <div className="grid grid-cols-2 gap-3 font-mono">
                        ...
                        <div className="bg-white/[0.02] border border-white/5 p-4 rounded-2xl">
                          ...
                          <p className="text-xl font-black italic mt-1 text-cyan-400">
                            {formatNumber(
                        ...
                        <div className="bg-white/[0.02] border border-white/5 p-4 rounded-2xl">
                          <p className="text-[8px] font-black text-zinc-500 uppercase tracking-widest">
                            Organic Target Likes
                          </p>

                          <p className="text-xl font-black italic mt-1 text-rose-500">
                            {formatNumber(
                      ...
                      <div className="grid grid-cols-2 gap-3 font-mono">
                        ...
                        <div className="bg-white/[0.02] border border-white/5 p-4 rounded-2xl">
                          ...
                          <p className="text-xl font-black italic mt-1 text-zinc-300">
                            64.2%
                          </p>
                        </div>
                        ...
                        <div className="bg-white/[0.02] border border-white/5 p-4 rounded-2xl">
                          ...
                          <p className="text-xl font-black italic mt-1 text-yellow-500">
                            0:42s
                          </p>
                        </div>
...
const StatCard = ({
  ...
  <div
    className={`bg-zinc-900/20 border border-white/5 p-5 rounded-[24px] bg-gradient-to-br ${color} to-transparent backdrop-blur-sm`}
  >
    <div
      className={`w-7 h-7 rounded-lg bg-black flex items-center justify-center mb-3 ${iconColor} border border-white/10 shadow-md`}
    >
      {icon}
    </div>

    <p className="text-xl font-black italic tracking-tight font-mono">
      {value}
    </p>

    <p className="text-[8px] font-bold text-zinc-500 uppercase mt-0.5 tracking-wider">
      {label}
    </p>
  </div>
);
  ...
  >
    ...
    <div className="flex-1 min-w-0">
      ...
      <div className="flex items-center gap-2">
        ...
        <h4 className="text-xs font-bold text-zinc-200 tracking-wide">
          {title}
        </h4>

        {badge && (
          <span className="text-[6.5px] font-black px-1.5 py-0.5 rounded bg-cyan-500 text-black uppercase tracking-wide">
            {badge}
          </span>
        )}
      ...
      <p className="text-[10px] text-zinc-500 mt-0.5 leading-tight truncate">
        {desc}
      </p>
